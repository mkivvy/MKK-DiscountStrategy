 ...
 * receipt in a retail system for the MKK-DiscountStrategy Project. <p> Receipt
 * contains an array of LineItem objects which print on the receipt displaying
 * the product, quantity, cost, and discount amount for each item purchased.
 * Header and footer information is also printed on the receipt including a
 * total amount the customer saved on this sale. <p> Receipt instantiates a
 * Customer object to retrieve customer information for the receipt footer to
 * display. <p> This class also contains a static receipt number which
 * increments for each new receipt and a store number with a default value. <p>
...
public class Receipt {
     ...
     * LineItem for the receipt. A Customer object is instantiated using the
     * customer id to gather customer information for the recept. The
     * issueGiftReceipt defaults to false so no gift receipts are issued. The
     * static field receipt number is incremented.
     *
     ...
     * Constructor instantiates the class using the product id, quantity, issue
     * gift receipt, and a discount strategy fields to instantiate the first
     * LineItem for the receipt. Customer id would need to be set later in the
     * process in order to display customer information on the receipt. The
     * issueGiftReceipt defaults to false so no gift receipts are issued. The
     * static field receipt number is incremented.
     *
     * @param prodId the product id, a unique string identifying the product.
     ...
     * gift receipt, and a discount strategy fields to instantiate the first
     * LineItem for the receipt. A Customer object is instantiated using the
     * customer id to gather customer information for the recept. The static
     * field receipt number is incremented.
     *
     * @param prodId the product id, a unique string identifying the product.
     * not null, not empty
     * @param qty the number of the item being purchased, not less than zero
     ...
     * Constructor instantiates the class using the product id, quantity, issue
     * gift receipt, and a discount strategy fields to instantiate the first
     * LineItem for the receipt. Discount strategy is not a required parameter
     * for LineItem; it is only passed along if the discount strategy is to be
     * set at the time of the sale. A Customer object is instantiated using the
     * customer id to gather customer information for the recept. The static
     * field receipt number is incremented.
     *
     * @param prodId the product id, a unique string identifying the product.
     ...
     * calculate the discount for this particular product. not null
     * @throws NullPointerException if the productId or the discStrategy
     * parameter is null
     * @throws IllegalArgumentException if the productId parameter is empty or
     * the qty parameter is less than zero or if the discStrategy parameter is
     * is not an instance of a DiscountStrategy type
     */
    public Receipt(String prodId, int qty, String custId,
     ...
     * LineItem for the receipt. Discount strategy is not a required parameter
     * for LineItem; it is only passed along if the discount strategy is to be
     * set at the time of the sale. Customer id would need to be set later in
     * the process in order to display customer information on the receipt. The
     * static field receipt number is incremented.
     *
     * @param prodId the product id, a unique string identifying the product.
     * not null, not empty
...
//        System.out.println(lineItems[0].getFormattedLineHeader());
        System.out.println(lineItems.get(0).getFormattedLineHeader());
    }
